...
for user in users:
    ...
    weights *= np.where(
        games_all.loc[candidate_indices, "Platform"].values == user["preferred_platform"],
        2.0,
        1.0,
    )
    weights = weights / weights.sum()
...
players = pd.DataFrame(
    {
        ...
        "avg_session_length_hours": [round(u["avg_session_length_hours"], 2) for u in users],
...
for user in users:
    ...
    score = np.where(g["Platform"] == user["preferred_platform"], 2.0, 1.0).astype(float)
